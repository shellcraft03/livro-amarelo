...
function getSecret() {
  ...
  if (process.env.TURNSTILE_SECRET) {
    if (process.env.NODE_ENV === 'production') {
      console.warn('[session] APP_SESSION_SECRET not set — falling back to TURNSTILE_SECRET');
    }
...
export function setHumanSessionCookie(req, res) {
  ...
  if (!secret) throw new Error('Missing env var: APP_SESSION_SECRET or TURNSTILE_SECRET');
